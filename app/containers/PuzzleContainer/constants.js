/*
 *
 * PuzzleContainer constants
 *
 */

export const DEFAULT_ACTION = 'app/PuzzleContainer/DEFAULT_ACTION';
export const PUZZLES_LOADED = 'app/PuzzleContainer/PUZZLES_LOADED';
export const PUZZLES_SAVED = 'app/PuzzleContainer/PUZZLES_SAVED';
export const PUZZLES_SAVED_SUCCESS =
  'app/PuzzleContainer/PUZZLES_SAVED_SUCCESS';
export const PUZZLE_CREATED = 'app/PuzzleContainer/PUZZLE_CREATED';
export const PUZZLE_SELECTED = 'app/PuzzleContainer/PUZZLE_SELECTED';
export const PUZZLE_UPLOADED = 'app/PuzzleContainer/PUZZLE_UPLOADED';
